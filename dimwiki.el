...
(defun dimwiki--read-from-minibuffer (prompt)
  "Read a value from the minibuffer with PROMPT.
If there's a string at point, offer that as a default."
  (let* ((suggested (dimwiki--dwim-at-point))
         (final-prompt
          (if suggested
...
(defun dimwiki-search (term)
  "Look up TERM in a wiki.
See `dimwiki-wikidomains' for what wiki's are searched."
  (interactive (list (dimwiki--read-from-minibuffer "Search string")))
